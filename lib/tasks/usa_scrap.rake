...
namespace :usa_scrap do
  desc "Scrap leagues in the region USA from flattrackstat"

  task scrap: :environment do

    # There are 5 pages on which we iterate
    [0, 1, 2, 3, 4, 5, 6, 7, 8].each do |page_number|
      url = "http://flattrackstats.com/teams/results/taxonomy%3A15%2C11%2C49?page=#{page_number}"
      ...
      html_doc.search('tr').each_with_index do |element, i|
        ...
        location = location.sub(/, [A-Z]{2}/, "")
        # LOCATION WITH ALGOLIA
        algolia_location = JSON.parse((RestClient.post "https://places-dsn.algolia.net/1/places/query", {'query' => "#{location}", type: "city",  countries: ["us"]}.to_json, {content_type: :json, accept: :json}))
        ...
        puts "CREATED #{League.where(region: "USA").count} LEAGUES!"
