...
namespace :pacific_scrap do
  desc "Scrap leagues in the region Pacific from flattrackstat"

  task scrap: :environment do

    # There are 5 pages on which we iterate
    [0].each do |page_number|
      url = "http://flattrackstats.com/teams/results/taxonomy%3A18%2C11%2C49?page=#{page_number}"
      ...
      html_doc.search('tr').each_with_index do |element, i|
        ...
        puts "CREATED #{League.where(region: "Pacific").count} LEAGUES!"
