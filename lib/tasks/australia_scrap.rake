...
namespace :australia_scrap do
  desc "Scrap leagues in the region Australia from flattrackstat"

  task scrap: :environment do

    # There are 5 pages on which we iterate
    [0].each do |page_number|
      url = "http://flattrackstats.com/teams/results/taxonomy%3A39%2C11%2C49?page=#{page_number}"
      ...
      html_doc.search('tr').each_with_index do |element, i|
        ...
        algolia_location = JSON.parse((RestClient.post "https://places-dsn.algolia.net/1/places/query", {'query' => "#{location}", type: "city",  countries: ["au"]}.to_json, {content_type: :json, accept: :json}))
        ...
        puts "CREATED #{League.where(region: "Australia").count} LEAGUES!"
